...
export class BeeyardController {
  ...
  async create(@Body() beeyardDto: BeeyardDto, @Res() res: Response) {
    try {
      ...
      res.status(HttpStatus.BAD_REQUEST).json({message: `Erreur dans la création du rucher : ${err.message}`});
    }
  }
  ...
  async update(@Body() beeyardDto: BeeyardDto, @Res() res: Response) {
    try {
      ...
      res.status(HttpStatus.BAD_REQUEST).json({message: `Erreur dans la mise a jour du rucher : ${err.message}`});
    }
  }
  ...
  async delete(@Param('id') id: string, @Res() res: Response) {
    ...
    if (isNaN(beeyardId)) {
      res.status(HttpStatus.BAD_REQUEST).json({message: 'Identifiant rucher invalide'});
    }
    try {
      await this.beeyardService.delete(beeyardId);
      res.status(HttpStatus.OK).json({message: 'Ruché supprimé avec succès.'});
    } catch (err: any) {
      res.status(HttpStatus.BAD_REQUEST).json({message: `Erreur dans la suppression du rucher : ${err.message}`});
    }
  }
  ...
  async findOne(@Param('id') id: string, @Res() res: Response) {
    ...
    try {
      ...
      res.status(HttpStatus.BAD_REQUEST).json({message: `Erreur dans la récupération du rucher : ${err.message}`});
    }
  }
  ...
  async findAllByIdBeekeeper(@Param('id') id: string, @Res() res: Response) {
    ...
    try {
      const beeyards: BeeyardDto[] = await this.beeyardService.findAllByIdBeekeeper(beekeeperId);
      res.status(HttpStatus.OK).json(beeyards);
    } catch (err: any) {
      res.status(HttpStatus.BAD_REQUEST).json({message: `Erreur dans la récupération des ruchers : ${err.message}`});
    }
  }
}
